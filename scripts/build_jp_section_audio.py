...
BASE_DIR = Path("/home/ps/SpeakingApp/mp3/jp")
...
def sentence_sort_key(path: Path) -> int:
    return int(path.stem.split("_", 1)[0])


def build_section(section_dir: Path) -> None:
    files = sorted(section_dir.glob("*_female.mp3"), key=sentence_sort_key)
    if len(files) != 10:
        raise RuntimeError(f"{section_dir} expected 10 files, got {len(files)}")
    ...
    out_path = section_dir / "all_sentences.mp3"
    out_path.write_bytes(encode_mp3(b"".join(chunks), sample_rate, channels, sample_width))


def main() -> None:
    for section_dir in sorted((p for p in BASE_DIR.iterdir() if p.is_dir()), key=lambda path: path.name):
        build_section(section_dir)
    print("done")
